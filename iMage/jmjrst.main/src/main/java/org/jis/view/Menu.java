...
public class Menu extends JMenuBar {
	...
	public Menu(Main m) {
		...
		java.util.List<JmjrstPlugin> pluginList = PluginManager.getPlugins();
		configurableItems = new JMenuItem[pluginList.size()];
		runableItems = new JMenuItem[pluginList.size()];
		for (int i = 0; i < pluginList.size(); i++) {
			JmjrstPlugin p = pluginList.get(i);
			JMenu plugin = new JMenu(pluginList.get(i).getName());
			if (pluginList.get(i).isConfigurable()) {
